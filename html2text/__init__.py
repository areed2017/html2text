...
class HTML2Text(html.parser.HTMLParser):
	...
	callback = self.tag_callback(self, tag, attrs, start) if callable(self.tag_callback) else False
        ...
        # Open tag's style that does not include anything from its parents
	...
	if callback:
        ...
        if tag == "pre" or "pre" in own_style.get("white-space", ""):
            if start:
